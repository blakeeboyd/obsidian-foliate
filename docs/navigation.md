...
The sidebar and command palette both offer ways to navigate between notes and find where taxa are mentioned.
...
Jumping is the default action for a plain click, but the click and each modifier-click are configurable in **Settings → Foliate → Click Actions**: plain click, **Cmd/Ctrl+click**, **Option/Alt+click**, and **Shift+click**. Each can jump in the document, open the note in the current tab, a new tab, Split View, or a new window, or copy a wikilink to the note. Defaults: click jumps, Shift opens in Split View, Cmd/Ctrl in the current tab, Option/Alt in a new tab. With several modifiers held, precedence is Cmd/Ctrl, then Option/Alt, then Shift.
...
- **Custom color:** set any color via the color picker in Settings → Foliate
...
Toggle the highlight in **Settings → Foliate → Highlight on jump**.
...
|---------|-------------|
| **Create taxa link** | Link selected text as a taxon (with prefix detection or picker). With no selection, falls back to the cursor (when "Link word under cursor" is enabled): links an existing taxa term whose span covers the cursor (the whole phrase, e.g. "artificial intelligence"), or the single word under the cursor when it matches a taxa file or carries a prefix. A word that matches nothing is left alone |
...
| **Open Foliate sidebar** | Show the Foliate panel in the right sidebar |
